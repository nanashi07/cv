...
擅長系統規劃與開發，提升開發流程，效率與產出品質。具有獨立作業及帶領成員完成目標的能力。    
主要使用語言為 Java 及 C#，並以 Python / node.js 等為輔助處理事務。熟悉但不限定於 Web 相關的系統，前端語言多數使用 Angularjs，Vue，Polymer，jQuery 等。    
熱愛學習技術，每天都會維持著閱讀各類文章的習慣，並對有興趣的內容進行較深入的了解。   
閒暇時間喜歡做自己的 side project，並將較完整的項目放至 Github 上。        
擁有自已架設維護的各種服務，包含 Gitlab, SonarQube, ELK 與其它網頁服務與手機應用程式等。
...
主要專案: VCMS, Vendor2G, SISO
相關技術: .net Web form, SQL Server, jQuery, Jenkins, SVN
Links: https://7-11vcms.pcsc.com.tw
```
